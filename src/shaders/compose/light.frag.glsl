...
void main() {
    ...
    if (ndotl > 0.0) {
        ...
        power *= (
                sampleShadow( ndir + offset.xyy, len, power) +
                sampleShadow( ndir + offset.yyy, len, power) +
                sampleShadow( ndir + offset.xyx, len, power) +
                sampleShadow( ndir + offset.yyx, len, power) +
                sampleShadow( ndir, len, power) +
                sampleShadow( ndir + offset.xxy, len, power) +
                sampleShadow( ndir + offset.yxy, len, power) +
                sampleShadow( ndir + offset.xxx, len, power) +
                sampleShadow( ndir + offset.yxx, len, power)
        ) * ( 1.0 / 9.0 );
    }
