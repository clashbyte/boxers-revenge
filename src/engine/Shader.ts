...
// type UniformValue = number | vec2 | vec3 | vec4 | mat2 | mat3 | mat4;
...
export class Shader {
   ...
   * @param offscreen
   */
  public constructor(fragCode: string, vertCode: string, offscreen: boolean = false) {
    ...
    try {
      ...
      this.fragmentShader = this.createShader(
        this.prefixFragmentCode(offscreen),
        GL.FRAGMENT_SHADER,
      );
   ...
   * @private
   */
  private prefixFragmentCode(offscreen: boolean) {
    return `${FragmentBase.replace(
      '// OFFSCREEN_PASS',
      offscreen ? '#define OFFSCREEN_PASS 1' : '',
    )}\n${this.fragmentCode}`;
  }
}
